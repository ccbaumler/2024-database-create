...
rule check_txt_reversioned:
    ...
    shell: """
        files=""
        for file in "data/updated-versions.*-{wildcards.D}-k*.csv"; do
            files+=" $file"
        done
...
rule gather_sketch_reversioned:
    ...
    threads: 1
    params:
        k_list = lambda wildcards: ",".join([f"k={k}" for k in wildcards]),
    shell:'''
        sourmash scripts gbsketch {input.reversion} -o {output.db} --failed {output.failed} \
            --params-str "dna,{params.k_list},scaled=1000,abund" -r 1 #2> {log}
    '''
...
rule check_txt_missing:
    input:
        reversion = expand("data/{D}.{k}.missing-genomes.csv", D=DOMAINS, k=KSIZES),
        script = "scripts/check_txt_files.py",
    output:
        solo = "data/{D}.missing-genomes.csv",
    shell: """
        files=""
        for file in "data/{wildcards.D}.*.missing-genomes.csv"; do
            files+=" $file"
        done
        {input.script} $files -o {output.solo}
    """
